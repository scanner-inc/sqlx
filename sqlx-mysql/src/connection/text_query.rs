...
//!
//! `interpolate` refuses any SQL containing `;` followed by another
//! statement (whitespace and comments after the `;` are fine — only
//! actual second-statement tokens trigger the error). The reason is that
//! `no_backslash_escape` (and implicitly the connection charset) is
//! captured once before the COM_QUERY is sent, but a multi-statement
//! query could flip `NO_BACKSLASH_ESCAPES` (or charset) in an earlier
//! statement, so the escaping mode used by `Value::as_sql` for later
//! statements would no longer match how the server parses them. That
//! mismatch is a string-literal break-out vector. Server-side
//! `COM_STMT_PREPARE` doesn't accept multi-statement either, so this
//! restriction matches the prepared-statement path's behavior.
//!
//! # Known limitations
//!
...
///
/// **Multi-statement queries are rejected** by this path (see the
/// `saw_separator` logic below). Reason: `no_backslash_escape` is captured
/// once from the connection status when `interpolate` is called, but a
/// multi-statement COM_QUERY can flip `NO_BACKSLASH_ESCAPES` (or
/// `ANSI_QUOTES`, or the connection charset) in an earlier statement, so
/// the escaping mode used by `Value::as_sql` for later statements would
/// no longer match how the server parses them — potentially allowing a
/// string-literal break-out. Server-side `COM_STMT_PREPARE` doesn't accept
/// multi-statement either, so this restriction matches the prepared-
/// statement path's behavior.
///
/// References:
...
fn splice(sql: &str, values: &[Value], no_backslash_escape: bool) -> Result<String, Error> {
    ...
    let mut i = 0usize;
    // Once we've seen a `;` in Normal state, any further non-whitespace,
    // non-comment Normal-state content means a second statement — refuse.
    // See the function's docstring for the security rationale.
    ...
    while i < bytes.len() {
        ...
        match state {
            State::Normal => match b {
                b'\'' | b'"' | b'`' => {
                    if saw_separator {
                        return Err(multi_statement_err());
                    }
                ...
                b';' => {
                    // Statement separator. By itself a trailing `;` is
                    // harmless (servers tolerate it on a single statement),
                    // but anything except whitespace or comments after this
                    // point would constitute a second statement, which we
                    // refuse — see the function's docstring for the
                    // sql_mode-flip security rationale.
                    saw_separator = true;
                    i += 1;
                }
                // Any other byte in Normal state is just a regular SQL
                // token character; no placeholder action. After we've seen
                // a statement separator, though, any non-whitespace token
                // means a second statement is starting.
                _ => {
                    if saw_separator && !b.is_ascii_whitespace() {
                        return Err(multi_statement_err());
                    }
                    i += 1;
                }
...
fn multi_statement_err() -> Error {
    err_protocol!(
        "interpolation: refusing multi-statement SQL on the COM_QUERY \
         text path. The connection's no_backslash_escape / charset state \
         is captured once before sending the query, so an earlier \
         statement that flips `NO_BACKSLASH_ESCAPES` (or `ANSI_QUOTES`, \
         or charset) would desync from how later statements are parsed \
         on the server, potentially allowing a string-literal break-out. \
         Submit one statement at a time, or use the prepared-statement \
         path (statement_cache_capacity > 0)."
    )
}
...
mod tests {
    ...
    #[test]
    fn splice_multi_statement_is_rejected() {
        // Multi-statement SQL is rejected because we can't safely capture
        // sql_mode / charset state once and re-use it across statements
        // that may flip those modes. See `splice` docstring.
        let args = args_with(|a| {
            a.add(1i32).unwrap();
            a.add(2i32).unwrap();
        });
        let err = interpolate("SELECT ?; SELECT ?", &args, false).unwrap_err();
        assert!(
            err.to_string().contains("multi-statement"),
            "unexpected error message: {err}"
        );
    }

    #[test]
    fn splice_multi_statement_set_sql_mode_is_rejected() {
        // The specific attack vector the rejection exists to close: an
        // earlier statement flipping NO_BACKSLASH_ESCAPES so our captured
        // escape mode no longer matches how the server will parse the
        // later statement.
        let args = args_with(|a| {
            a.add("o'reilly").unwrap();
        });
        let err = interpolate(
            "SET sql_mode='NO_BACKSLASH_ESCAPES'; SELECT ?",
            &args,
            false,
        )
        .unwrap_err();
    ...
    #[test]
    fn splice_trailing_semicolon_is_ok() {
        // A bare trailing `;` with nothing meaningful after it is fine —
        // single statement.
        let args = args_with(|a| {
            a.add(1i32).unwrap();
        });
    ...
    #[test]
    fn splice_trailing_semicolon_with_whitespace_is_ok() {
        let args = args_with(|a| {
            a.add(1i32).unwrap();
        });
        let out = interpolate("SELECT ?;   \n  ", &args, false).unwrap();
        assert_eq!(out, "SELECT 1;   \n  ");
    }

    #[test]
    fn splice_trailing_semicolon_then_comment_is_ok() {
        // Trailing line / block comments after the `;` are not a second
        // statement.
        let args = args_with(|a| {
            a.add(1i32).unwrap();
        });
        let out =
            interpolate("SELECT ?; -- bye\n# done\n /* end */", &args, false).unwrap();
        assert_eq!(out, "SELECT 1; -- bye\n# done\n /* end */");
    }

    #[test]
    fn splice_double_semicolon_is_ok() {
        let args = args_with(|a| {
            a.add(1i32).unwrap();
        });
        let out = interpolate("SELECT ?;;", &args, false).unwrap();
    ...
    #[test]
    fn splice_semicolon_inside_string_is_ok() {
        // A `;` inside a string literal is just data, not a separator.
        let args = args_with(|a| {
            a.add(1i32).unwrap();
        });
        let out = interpolate("SELECT 'a;b' = ?", &args, false).unwrap();
    ...
    #[test]
    fn splice_semicolon_inside_block_comment_is_ok() {
        let args = args_with(|a| {
            a.add(1i32).unwrap();
        });
        let out = interpolate("SELECT 1 /* ; */ = ?", &args, false).unwrap();
        assert_eq!(out, "SELECT 1 /* ; */ = 1");
    }

    #[test]
    fn splice_semicolon_inside_line_comment_is_ok() {
        let args = args_with(|a| {
            a.add(1i32).unwrap();
        });
        let out = interpolate("SELECT 1 -- ;\n = ?", &args, false).unwrap();
    ...
    #[test]
    fn splice_semicolon_inside_backticks_is_ok() {
        let args = args_with(|a| {
            a.add(1i32).unwrap();
        });
        let out = interpolate("SELECT `weird;name` = ?", &args, false).unwrap();
        assert_eq!(out, "SELECT `weird;name` = 1");
    }
